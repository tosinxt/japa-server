...
import { Job_category, Job_type, Jobs, Technologies, Years_of_experience, } from "../Models/jobs.js";
...
export const login_admin = async_runner(async (req, res) => {
...
export const talent_list = async_runner(async (req, res) => {
    const { name, page = 1, limit = 100, } = matchedData(req, { locations: ["query"] });
    const filter = {};
    if (name)
...
export const post_jobs = async_runner(async (req, res) => {
    const role = req.params.role;
    if (role === "admin" || role === "super_admin") {
        const { job_title, location, job_type, company_name, salary_range, experience, about, what_you_will_be_doing, what_we_are_lookin_for, category, applicants, link, } = matchedData(req);
...
export const edit_jobs = async_runner(async (req, res) => {
    const role = req.params.role;
    if (role === "admin" || role === "super_admin") {
        const { job_title, location, job_type, company_name, salary_range, experience, about, what_you_will_be_doing, what_we_are_lookin_for, category, applicants, link, job_id, } = matchedData(req);
        const update_job = await Jobs.findByIdAndUpdate(job_id, {
            job_title,
            location,
            job_type,
            company_name,
...
export const delete_jobs = async_runner(async (req, res) => {
    const { _id } = req.body;
...
export const post_job_category = async_runner(async (req, res) => {
    const { name } = req.body;
    const role = req.params.role;
    if (role === "admin" || role === "super_admin") {
...
export const post_job_type = async_runner(async (req, res) => {
    const { name } = req.body;
    const role = req.params.role;
    if (role === "admin" || role === "super_admin") {
...
export const post_technology = async_runner(async (req, res) => {
    const { name } = req.body;
    const role = req.params.role;
    if (role === "admin" || role === "super_admin") {
...
export const post_years_of_experience = async_runner(async (req, res) => {
    const { name } = req.body;
    const role = req.params.role;
    if (role === "admin" || role === "super_admin") {
...
export const list_job_cats = async_runner(async (req, res) => {
...
export const list_job_type = async_runner(async (req, res) => {
...
export const list_technologies = async_runner(async (req, res) => {
...
export const list_yoe = async_runner(async (req, res) => {
...
export const post_courses = async_runner(async (req, res) => {
    const role = req.params.role;
    if (role === "admin" || role === "super_admin") {
        const { title, about, course_outline, over_view, link, requirements } = matchedData(req);
...
export const edit_courses = async_runner(async (req, res) => {
    const role = req.params.role;
    if (role === "admin" || role === "super_admin") {
        const { title, about, course_outline, over_view, link, requirements, course_id, } = matchedData(req);
        const update_course = await Courses.findByIdAndUpdate(course_id, {
            title,
            about,
            link,
            course_outline,
...
export const delete_course = async_runner(async (req, res) => {
    const { _id } = req.body;
...
export const stats = async_runner(async (req, res) => {
...
export const user_list = async_runner(async (req, res) => {
    const { name, email, page = 1, limit = 100, } = matchedData(req, { locations: ["query"] });
    const filter = {};
    if (name)
...
export const jobs_list = async_runner(async (req, res) => {
    const { title, page = 1, limit = 100, } = matchedData(req, { locations: ["query"] });
    const filter = {};
    if (title)
...
export const course_list = async_runner(async (req, res) => {
    const { title, page = 1, limit = 100, } = matchedData(req, { locations: ["query"] });
    const filter = {};
    if (title)
        filter.job_title = { $regex: title, $options: "i" };
